...
pub async fn fetch_posts(db_pool: &DBPool, search: Option<String>) -> Result<Vec<Post>> {
    let con = get_db_con(db_pool).await?;
...
pub async fn create_post(db_pool: &DBPool, body: PostRequest) -> Result<Post> {
    let con = get_db_con(db_pool).await?;
...
pub async fn update_post(db_pool: &DBPool, id: i32, body: PostUpdateRequest) -> Result<Post> {
    let con = get_db_con(db_pool).await?;
    let query = format!(
...
pub async fn delete_post(db_pool: &DBPool, id: i32) -> Result<u64> {
    let con = get_db_con(db_pool).await?;
